...
pub async fn create(db: &DatabaseConnection, draft: EventRuleDraft) -> Result<EventRuleInfo, DbError> {
    ...
    let row = event_rule::ActiveModel {
        ...
        config: Set(serde_json::to_string(&draft.config).map_err(|e| DbError::Validation(e.to_string()))?),
        created_at: Set(now),
        updated_at: Set(now),
        ..Default::default()
    }
        ...
        if let Some(last) = row.last_fired_at {
            ...
            if elapsed > ATTEMPT_CHAIN_IDLE_RESET {
                ...
                append_log(
                    ...
                    conversation_id,
                    "skipped_cooldown",
                    Some(format!("cooldown_ms={cooldown_ms}")),
                )
                .await?;
