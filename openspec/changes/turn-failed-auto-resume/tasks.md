...
- [x] 1.2 Phase 1B 必需：由 event-automation-ui 交付 keywords/ANY/ALL/regex/error_kind/prompt/guards/模板编辑双入口，验证 UI 编辑后真实发送行为改变；不得 deferred。
...
- [ ] 3.1 配合 event-automation-ui 验证模板修改/重启保留、当前会话隔离、三次上限和 cooldown，记录 Desktop/Web 消息与日志证据（Web 已完成；Desktop visual interaction `NOT_PROVEN`）。
